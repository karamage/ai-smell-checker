...
- 貼った瞬間に解析。12 のルールで指摘箇所がエディタ上でルール別に色付けされ、クリックすると該当箇所に飛ぶ
...
静的解析のルールは 12 個。R0〜R9 の元ネタは「AI 臭い文章」を見分けるための実務チェックリスト。R10 は [natural-japanese](https://github.com/coji/natural-japanese) の lint にある統計系検出器（burstiness、文長の自己相関、段落構造の均質さ）を、形態素解析なしで動くように文字数ベースで移植したもの。
