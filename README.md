...
- 指摘を元に「AI に直させる」ための制約付きプロンプトを生成。言い換えで別の常套句や比喩が湧かないよう、入れてはいけない表現を添える。コピーするか ChatGPT / Claude にそのまま渡せる
...
bun run cli draft.md --prompt | pbcopy   # 書き直し用プロンプトをクリップボードへ
```
